...
static void sn_pairing(element_ptr out, element_ptr in1, element_ptr in2,
    pairing_t pairing) {
	sn_pairing_data_ptr p = (sn_pairing_data_ptr)pairing->data;
  element_ptr Q = in2;
  element_t e0;
  element_t R, QR;
  element_init(R, p->Eq);
...
void pairing_init_singular_with_node(pairing_t pairing, mpz_t q) {
  ...
  pairing->data = pbc_malloc(sizeof(sn_pairing_data_t));
  p = (sn_pairing_data_ptr)pairing->data;
