...
static void g_clear(void *data) {
	g_param_ptr param = (g_param_ptr)data;
	int i;
	mpz_clear(param->q);
	mpz_clear(param->n);
	mpz_clear(param->h);
...
static void g_out_str(FILE *stream, void *data) {
	g_param_ptr p = (g_param_ptr)data;
	int i;
	char s[8];
...
static void tatepower10(element_ptr out, element_ptr in, pairing_t pairing) {
	mnt_pairing_data_ptr p = (mnt_pairing_data_ptr)pairing->data;
	...
	element_t *inre = (element_t*)element_x(in)->data;
	element_t *inim = (element_t*)element_y(in)->data;
	//see thesis
#define qpower(sign) {                         \
...
static void g_pairing_pp_init(pairing_pp_t p, element_ptr in1, pairing_t pairing) {
	...
	element_t Z;
	int m;
	mnt_pairing_data_ptr info = (mnt_pairing_data_ptr)pairing->data;
	element_t t0;
	element_t a, b, c;
...
static void g_pairing_pp_apply(element_ptr out, element_ptr in2, pairing_pp_t p) {
	mpz_ptr q = p->pairing->r;
	mnt_pairing_data_ptr info = (mnt_pairing_data_ptr)p->pairing->data;
	...
	//map fr6om twist: (x, y) --> (v^-1 x, v^-(3/2) y)
	//where v is the quadratic nonresidue used to construct the twist
...
static void g_finalpow(element_ptr e) {
	...
	tatepower10(t0, (element_s*)e->data, e->field->pairing);
	element_set((element_s*)e->data, t0);
	element_clear(t0);
}
...
static void g_init_pairing(pairing_t pairing, void *data) {
	...
	pairing->data = pbc_malloc(sizeof(mnt_pairing_data_t));
	p = (mnt_pairing_data_ptr)pairing->data;
	...
	p->Fqd->nqr = (element_ptr)pbc_malloc(sizeof(element_t));
...
int pbc_param_init_g(pbc_param_ptr par, struct symtab_s *tab) {
	g_init(par);
	g_param_ptr p = (g_param_ptr)par->data;
	char s[80];
	...
	p->coeff = (mpz_t*)pbc_realloc(p->coeff, sizeof(mpz_t)* 5);
	int i;
	for (i = 0; i < 5; i++) {
		sprintf(s, "coeff%d", i);
		mpz_init(p->coeff[i]);
...
void pbc_param_init_g_gen(pbc_param_t p, pbc_cm_ptr cm) {
	g_init(p);
	g_param_ptr param = (g_param_ptr)p->data;
	field_t Fq, Fqx, Fqd;
	element_t irred, nqr;
	int i;
	...
	param->coeff = (mpz_t*)pbc_realloc(param->coeff, sizeof(mpz_t)* 5);
