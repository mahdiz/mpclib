...
static void e_clear(void *data) {
	e_param_ptr ep = (e_param_ptr)data;
	mpz_clear(ep->q);
	mpz_clear(ep->r);
	mpz_clear(ep->h);
	mpz_clear(ep->a);
...
static void e_out_str(FILE *stream, void *data) {
	e_param_ptr p = (e_param_ptr)data;
...
static void e_pairing(element_ptr out, element_ptr in1, element_ptr in2,
	pairing_t pairing) {
	e_pairing_data_ptr p = (e_pairing_data_ptr)pairing->data;
	element_ptr Q = in2;
	element_t QR;
...
static void e_finalpow(element_ptr e) {
	element_pow_mpz((element_s*)e->data, (element_s*)e->data, e->field->pairing->phikonr);
}

static void e_init_pairing(pairing_t pairing, void *data) {
	e_param_ptr param = (e_param_ptr)data;
	e_pairing_data_ptr p;
	element_t a, b;
	...
	pairing->data = pbc_malloc(sizeof(e_pairing_data_t));
	p = (e_pairing_data_ptr)pairing->data;
	p->exp2 = param->exp2;
	p->exp1 = param->exp1;
	p->sign1 = param->sign1;
	p->sign0 = param->sign0;
...
static void e_init(pbc_param_ptr p) {
	...
	p->api = interface;
	e_param_ptr ep;
	p->data = pbc_malloc(sizeof(*ep));
	ep = (e_param_ptr)p->data;
	mpz_init(ep->q);
	mpz_init(ep->r);
	mpz_init(ep->h);
	mpz_init(ep->a);
...
int pbc_param_init_e(pbc_param_ptr par, struct symtab_s *tab) {
	e_init(par);
	e_param_ptr p = (e_param_ptr)par->data;
