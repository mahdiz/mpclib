...
static void a_out_str(FILE *stream, void *data) {
	a_param_ptr p = (a_param_ptr)data;
...
static void a_clear(void *data) {
	a_param_ptr sp = (a_param_ptr)data;
  mpz_clear(sp->r);
  mpz_clear(sp->q);
  mpz_clear(sp->h);
  pbc_free(data);
...
static void a_pairing_pp_init(pairing_pp_t p, element_ptr in1, pairing_t pairing) {
  int i, n;
  a_pairing_data_ptr ainfo = (a_pairing_data_ptr)pairing->data;
...
static void a_pairing_pp_clear(pairing_pp_t p) {
	a_pairing_data_ptr ainfo = (a_pairing_data_ptr)p->pairing->data;
  pp_coeff_t *coeff = (pp_coeff_t *) p->data;
...
static void a_pairing_ellnet_pp_init(pairing_pp_t p, element_ptr in1, pairing_t pairing) {
  ...
  p->data = pbc_malloc(sizeof(ellnet_pp_t));
  ellnet_pp_ptr pp = (ellnet_pp_ptr)p->data;
  pp->seq = (ellnet_pp_st_t *)pbc_malloc(sizeof(ellnet_pp_st_t)* rbits);
...
static void a_pairing_proj(element_ptr out, element_ptr in1, element_ptr in2,
    pairing_t pairing) {
	a_pairing_data_ptr p = (a_pairing_data_ptr)pairing->data;
  element_t V, V1;
  element_t z, z2;
  element_t f, f0, f1;
  element_t a, b, c;
...
void a_pairings_affine(element_ptr out, element_t in1[], element_t in2[],
	...
	a_pairing_data_ptr p = (a_pairing_data_ptr)pairing->data;
	element_t* V = (element_t*)pbc_malloc(sizeof(element_t)*n_prod);
	element_t* V1 = (element_t*)pbc_malloc(sizeof(element_t)*n_prod);
  element_t f, f0, f1;
  element_t a, b, c;
  element_t e0;
  int i, j, n;
...
static void a_finalpow(element_t e) {
  ...
  element_t t0, t1;
  element_init_same_as(t0, (element_s*)e->data);
  element_init_same_as(t1, (element_s*)e->data);
  a_tateexp(t0, (element_ptr)e->data, t1, pairing->phikonr);
  element_set((element_ptr)e->data, t0);
  element_clear(t0);
  element_clear(t1);
}

static void a_init_pairing(pairing_ptr pairing, void *data) {
	a_param_ptr param = (a_param_ptr)data;
  element_t a, b;
  a_pairing_data_ptr p;

  pairing->data = pbc_malloc(sizeof(*p));
  p = (a_pairing_data_ptr)pairing->data;
  p->exp2 = param->exp2;
  p->exp1 = param->exp1;
  p->sign1 = param->sign1;
  mpz_init(pairing->r);
...
static void a_param_init(pbc_param_ptr par) {
  ...
  par->api = interface;
  a_param_ptr p;
  par->data = pbc_malloc(sizeof(*p));
  p = (a_param_ptr)par->data;
  mpz_init(p->r);
  mpz_init(p->q);
  mpz_init(p->h);
}
...
int pbc_param_init_a(pbc_param_ptr par, struct symtab_s *tab) {
  a_param_init(par);
  a_param_ptr p = (a_param_ptr)par->data;
...
void pbc_param_init_a_gen(pbc_param_ptr par, int rbits, int qbits) {
  a_param_init(par);
  a_param_ptr sp = (a_param_ptr)par->data;
  int found = 0;
...
static void a1_clear(void *data) {
	a1_param_ptr param = (a1_param_ptr)data;
  mpz_clear(param->p);
  mpz_clear(param->n);
  pbc_free(data);
}
...
static void a1_out_str(FILE *stream, void *data) {
	a1_param_ptr p = (a1_param_ptr)data;
...
static void a1_pairing_pp_init(pairing_pp_t p, element_ptr in1, pairing_t pairing) {
  ...
  a1_pairing_data_ptr a1info = (a1_pairing_data_ptr)pairing->data;
  ...
  for(;;) {
    ...
    if (mpz_tstbit(pairing->r, m)) {
      ...
      *pp = pbc_malloc(sizeof(pp2_coeff_t));
      pp2_coeff_set((pp2_coeff_ptr)*pp, a, b, c2, e0, e1, c);
    } else {
      *pp = pbc_malloc(sizeof(pp_coeff_t));
	  pp_coeff_set((pp_coeff_ptr)*pp, a, b, c);
    }
    pp++;
    m--;
  }
...
static void a1_pairing_pp_apply(element_ptr out, element_ptr in2, pairing_pp_t p) {
  void **pp = (void**)p->data;
  a1_pairing_data_ptr a1info = (a1_pairing_data_ptr)p->pairing->data;
  element_t f, f0;
  element_t e0, e1;
  int m;
  element_ptr Qx = curve_x_coord(in2);
  ...
  #define do_tangent()                                   \
  pp_coeff_ptr ppp = (pp_coeff_ptr)*pp;                              \
    a_miller_evalfn(f0, ppp->a, ppp->b, ppp->c, Qx, Qy);

  #define do_line() {                                \
  pp2_coeff_ptr ppp = (pp2_coeff_ptr)*pp;                         \
    /*we'll map Q via (x,y) --> (-x, iy) */          \
...
static void a1_pairing_proj(element_ptr out, element_ptr in1, element_ptr in2,
    pairing_t pairing) {
	a1_pairing_data_ptr p = (a1_pairing_data_ptr)pairing->data;
  element_t V;
  element_t z, z2;
  element_t f, f0;
  element_t a, b, c;
...
static void a1_init_pairing(pairing_t pairing, void *data) {
  ...
  pairing->data = pbc_malloc(sizeof(a1_pairing_data_t));
  p = (a1_pairing_data_ptr)pairing->data;
  ...
  pairing->finalpow = a_finalpow;
  pairing->G1 = (field_ptr)pbc_malloc(sizeof(field_t));
...
static void a1_init(pbc_param_t p) {
  ...
  p->api = interface;
  a1_param_ptr param;
  p->data = pbc_malloc(sizeof(*param));
  param = (a1_param_ptr)p->data;
  mpz_init(param->p);
  mpz_init(param->n);
}
...
int pbc_param_init_a1(pbc_param_ptr par, struct symtab_s *tab) {
  a1_init(par);
  a1_param_ptr p = (a1_param_ptr)par->data;
...
void pbc_param_init_a1_gen(pbc_param_ptr par, mpz_t order) {
  a1_init(par);
  a1_param_ptr param = (a1_param_ptr)par->data;
