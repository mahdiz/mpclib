...
static void d_clear(void *data) {
	d_param_ptr param = (d_param_ptr)data;
  int d = param->k / 2;
  int i;
  mpz_clear(param->q);
  mpz_clear(param->n);
...
static void d_out_str(FILE *stream, void *data) {
	d_param_ptr p = (d_param_ptr)data;
  int d = p->k / 2;
  int i;
  char s[8];
...
static void cc_finalpow(element_t e) {
	cc_tatepower((element_ptr)e->data, (element_ptr)e->data, e->field->pairing);
}
...
static void cc_millers_no_denom_affine(element_t res, mpz_t q, element_t P[],
  ...
  element_ptr Px, Py;
  element_t* Z = (element_t*)pbc_malloc(sizeof(element_t)*n_prod);
  element_ptr Zx, Zy;
...
void cc_pairings_affine(element_ptr out, element_t in1[], element_t in2[],
  ...
  element_ptr Qbase;
  element_t* Qx = (element_t*)pbc_malloc(sizeof(element_t)*n_prod);
  element_t* Qy = (element_t*)pbc_malloc(sizeof(element_t)*n_prod);
  pptr p = (pptr)pairing->data;
  int i;
  for(i=0; i<n_prod; i++){
...
int pbc_param_init_d(pbc_param_ptr par, struct symtab_s *tab) {
  d_param_init(par);
  d_param_ptr p = (d_param_ptr)par->data;
  char s[80];
  int i, d;
  ...
  d = p->k / 2;
  p->coeff = (mpz_t*)pbc_realloc(p->coeff, sizeof(mpz_t) * d);
  for (i=0; i<d; i++) {
    sprintf(s, "coeff%d", i);
    mpz_init(p->coeff[i]);
...
void pbc_param_init_d_gen(pbc_param_ptr p, pbc_cm_ptr cm) {
  ...
  param->coeff = (mpz_t*)pbc_realloc(param->coeff, sizeof(mpz_t) * d);
