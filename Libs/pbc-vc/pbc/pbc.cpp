...
static val_ptr eval_id(tree_ptr t) {
	val_ptr x = (val_ptr)symtab_at(reserved, t->id);
	if (!x) x = (val_ptr)symtab_at(tab, t->id);
	if (!x) {
...
tree_ptr tree_new(val_ptr(*eval)(tree_ptr)) {
	tree_ptr res = (tree_ptr)pbc_malloc(sizeof(*res));
	res->eval = eval;
	return res;
}
...
int yywrap_readline(void) {
	...
	currentline = (char*)realloc(currentline, n + 2);
	currentline[n] = '\n';
	currentline[n + 1] = '\0';
