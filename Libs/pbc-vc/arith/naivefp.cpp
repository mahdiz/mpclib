...
static void zp_set_si(element_ptr e, signed long int op) {
	mpz_set_si((mpz_ptr)e->data, op);
	mpz_mod((mpz_ptr)e->data, (mpz_ptr)e->data, e->field->order);
}

static void zp_set_mpz(element_ptr e, mpz_ptr z) {
	mpz_set((mpz_ptr)e->data, z);
	mpz_mod((mpz_ptr)e->data, (mpz_ptr)e->data, e->field->order);
}

static void zp_set0(element_ptr e) {
	mpz_set_si((mpz_ptr)e->data, 0);
}

static void zp_set1(element_ptr e) {
	mpz_set_si((mpz_ptr)e->data, 1);
}

static size_t zp_out_str(FILE * stream, int base, element_ptr e) {
	return mpz_out_str(stream, base, (mpz_ptr)e->data);
}
...
static int zp_set_str(element_ptr e, const char *s, int base) {
	int result = pbc_mpz_set_str((mpz_ptr)e->data, s, base);
  mpz_mod((mpz_ptr)e->data, (mpz_ptr)e->data, e->field->order);
  return result;
}

static int zp_sgn_odd(element_ptr a) {
	mpz_ptr z = (mpz_ptr)a->data;
...
static int zp_sgn_even(element_ptr a) {
  ...
  if (mpz_is0((mpz_ptr)a->data)) {
    res = 0;
  } else {
	  mpz_add(z, (mpz_ptr)a->data, (mpz_ptr)a->data);
...
static void zp_add(element_ptr n, element_ptr a, element_ptr b) {
   ...
   */
  //This seems faster:
	mpz_add((mpz_ptr)n->data, (mpz_ptr)a->data, (mpz_ptr)b->data);
	if (mpz_cmp((mpz_ptr)n->data, n->field->order) >= 0) {
		mpz_sub((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
  }
}
...
static void zp_sub(element_ptr n, element_ptr a, element_ptr b) {
  ...
  //mpz_mod(n->data, n->data, n->field->order);
	mpz_sub((mpz_ptr)n->data, (mpz_ptr)a->data, (mpz_ptr)b->data);
  if (mpz_sgn((mpz_ptr) n->data) < 0) {
	  mpz_add((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
  }
}
...
static void zp_double(element_ptr n, element_ptr a) {
  //mpz_add(n->data, a->data, a->data);
	mpz_mul_2exp((mpz_ptr)n->data, (mpz_ptr)a->data, 1);
	if (mpz_cmp((mpz_ptr)n->data, n->field->order) >= 0) {
		mpz_sub((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
  }
}
...
static void zp_halve(element_ptr n, element_ptr a) {
	mpz_ptr z = (mpz_ptr)a->data;
  if (mpz_odd_p(z)) {
	  mpz_add((mpz_ptr)n->data, z, a->field->order);
	  mpz_tdiv_q_2exp((mpz_ptr)n->data, (mpz_ptr)n->data, 1);
  } else {
	  mpz_tdiv_q_2exp((mpz_ptr)n->data, (mpz_ptr)a->data, 1);
  }
}

static void zp_mul(element_ptr n, element_ptr a, element_ptr b) {
	mpz_mul((mpz_ptr)n->data, (mpz_ptr)a->data, (mpz_ptr)b->data);
	mpz_mod((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
}

static void zp_mul_mpz(element_ptr n, element_ptr a, mpz_ptr z) {
	mpz_mul((mpz_ptr)n->data, (mpz_ptr)a->data, z);
	mpz_mod((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
}

static void zp_mul_si(element_ptr n, element_ptr a, signed long int z) {
	mpz_mul_si((mpz_ptr)n->data, (mpz_ptr)a->data, z);
	mpz_mod((mpz_ptr)n->data, (mpz_ptr)n->data, n->field->order);
}

static void zp_pow_mpz(element_ptr n, element_ptr a, mpz_ptr z) {
	mpz_powm((mpz_ptr)n->data, (mpz_ptr)a->data, z, n->field->order);
}

static void zp_set(element_ptr n, element_ptr a) {
	mpz_set((mpz_ptr)n->data, (mpz_ptr)a->data);
}

static void zp_neg(element_ptr n, element_ptr a) {
	if (mpz_is0((mpz_ptr)a->data)) {
		mpz_set_ui((mpz_ptr)n->data, 0);
  } else {
		mpz_sub((mpz_ptr)n->data, n->field->order, (mpz_ptr)a->data);
  }
}

static void zp_invert(element_ptr n, element_ptr a) {
	mpz_invert((mpz_ptr)n->data, (mpz_ptr)a->data, n->field->order);
}

static void zp_random(element_ptr n) {
	pbc_mpz_random((mpz_ptr)n->data, n->field->order);
}

static void zp_from_hash(element_ptr n, void *data, int len) {
	pbc_mpz_from_hash((mpz_ptr)n->data, n->field->order, (unsigned char*)data, len);
}
...
static int zp_is_sqr(element_ptr a) {
  //0 is a square
	if (mpz_is0((mpz_ptr)a->data)) return 1;
	return mpz_legendre((mpz_ptr)a->data, a->field->order) == 1;
}
...
static int zp_to_bytes(unsigned char *data, element_t e) {
  ...
  pbc_mpz_out_raw_n(data, n, (mpz_ptr)e->data);
  return n;
}
