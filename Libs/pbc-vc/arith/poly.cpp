...
static void poly_alloc(element_ptr e, int n) {
	pfptr pdp = (pfptr)e->field->data;
  peptr p = (peptr)e->data;
  element_ptr e0;
  int k = p->coeff->count;
  while (k < n) {
	  e0 = (element_ptr)pbc_malloc(sizeof(element_t));
  ...
  while (k > n) {
    k--;
	e0 = (element_ptr)darray_at(p->coeff, k);
    element_clear(e0);
    pbc_free(e0);
...
static void poly_remove_leading_zeroes(element_ptr e) {
  ...
  while (n >= 0) {
    element_ptr e0 = (element_ptr)p->coeff->item[n];
...
static int poly_is1(element_ptr e) {
	peptr p = (peptr)e->data;
  if (p->coeff->count == 1) {
	  return element_is1((element_ptr)p->coeff->item[0]);
  }
  return 0;
}
...
static void poly_set_mpz(element_ptr e, mpz_ptr op) {
  ...
  poly_alloc(e, 1);
  element_set_mpz((element_ptr)p->coeff->item[0], op);
  poly_remove_leading_zeroes(e);
}
...
static void poly_set_multiz(element_ptr e, multiz op) {
  ...
  int i;
  for(i = 0; i < n; i++) {
	  element_set_multiz((element_ptr)p->coeff->item[i], multiz_at(op, i));
  }
  poly_remove_leading_zeroes(e);
}

static void poly_set(element_ptr dst, element_ptr src) {
	peptr psrc = (peptr)src->data;
	peptr pdst = (peptr)dst->data;
  int i;

  poly_alloc(dst, psrc->coeff->count);
  for (i=0; i<psrc->coeff->count; i++) {
	  element_set((element_ptr)pdst->coeff->item[i], (element_ptr)psrc->coeff->item[i]);
  }
}
...
static void poly_neg(element_ptr f, element_ptr g) {
	peptr pf = (peptr)f->data;
	peptr pg = (peptr)g->data;
  int i, n;

  n = pg->coeff->count;
  poly_alloc(f, n);
  for (i=0; i<n; i++) {
	  element_neg((element_ptr)pf->coeff->item[i], (element_ptr)pg->coeff->item[i]);
  }
}
...
static void poly_double(element_ptr f, element_ptr g) {
  ...
  poly_alloc(f, n);
  for (i=0; i<n; i++) {
	  element_double((element_ptr)pf->coeff->item[i], (element_ptr)pg->coeff->item[i]);
  }
}
...
static void poly_mul_mpz(element_ptr f, element_ptr g, mpz_ptr z) {
  ...
  poly_alloc(f, n);
  for (i=0; i<n; i++) {
	  element_mul_mpz((element_ptr)pf->coeff->item[i], (element_ptr)pg->coeff->item[i], z);
  }
}
...
static void poly_mul_si(element_ptr f, element_ptr g, signed long int z) {
  ...
  poly_alloc(f, n);
  for (i=0; i<n; i++) {
	  element_mul_si((element_ptr)pf->coeff->item[i], (element_ptr)pg->coeff->item[i], z);
  }
}
...
static void poly_mul(element_ptr r, element_ptr f, element_ptr g) {
  ...
  pprod = (peptr)prod->data;
  n = fcount + gcount - 1;
  poly_alloc(prod, n);
  element_init(e0, pdp->field);
  for (i=0; i<n; i++) {
	  element_ptr x = (element_ptr)pprod->coeff->item[i];
    element_set0(x);
    for (j=0; j<=i; j++) {
      if (j < fcount && i - j < gcount) {
		  element_mul(e0, (element_ptr)pf->coeff->item[j], (element_ptr)pg->coeff->item[i - j]);
        element_add(x, x, e0);
      }
    }
  }
...
static void polymod_random(element_ptr e) {
	element_t *coeff = (element_t *)e->data;
...
static void poly_div(element_ptr quot, element_ptr rem,
  ...
  while (k >= 0) {
    qe = (element_ptr)pq->coeff->item[k];
	element_mul(qe, binv, (element_ptr)pr->coeff->item[m]);
    for (i=0; i<=n; i++) {
      element_mul(e0, qe, poly_coeff(b, i));
	  element_sub((element_ptr)pr->coeff->item[i + k], (element_ptr)pr->coeff->item[i + k], e0);
    }
    k--;
    m--;
  }
...
static int polymod_to_bytes(unsigned char *data, element_t f) {
	mfptr p = (mfptr)f->field->data;
	element_t *coeff = (element_t*)f->data;
  int i, n = p->n;
  int len = 0;
  for (i=0; i<n; i++) {
...
static void polymod_init(element_t e) {
  ...
  int n = p->n;
  element_t *coeff;
  e->data = pbc_malloc(sizeof(element_t)* n);
  coeff = (element_t*)e->data;
...
static void polymod_set(element_t e, element_t f) {
	mfptr p = (mfptr)e->field->data;
	element_t *dst = (element_t *)e->data, *src = (element_t *)f->data;
  int i, n = p->n;
  for (i=0; i<n; i++) {
...
static int polymod_cmp(element_ptr f, element_ptr g) {
	mfptr p = (mfptr)f->field->data;
	element_t *c1 = (element_t *)f->data, *c2 = (element_t *)g->data;
  int i, n = p->n;
  for (i=0; i<n; i++) {
...
static void polymod_add(element_t r, element_t e, element_t f) {
	mfptr p = (mfptr)r->field->data;
	element_t *dst = (element_t *)r->data, *s1 = (element_t *)e->data, *s2 = (element_t *)f->data;
  int i, n = p->n;
  for (i=0; i<n; i++) {
...
static void polymod_double(element_t r, element_t f) {
	mfptr p = (mfptr)r->field->data;
	element_t *dst = (element_t *)r->data, *s1 = (element_t *)f->data;
  int i, n = p->n;
  for (i=0; i<n; i++) {
...
static void polymod_mul_degree3(element_ptr res, element_ptr e, element_ptr f) {
	mfptr p = (mfptr)res->field->data;
	element_t *dst = (element_t *)res->data, *s1 = (element_t *)e->data, *s2 = (element_t *)f->data;
  element_t c3, c4;
  element_t p0;
  ...
  kar_poly_2(dst, c3, c4, s1, s2, (element_t *)p0->data);
...
static void polymod_mul_degree6(element_ptr res, element_ptr e, element_ptr f) {
	mfptr p = (mfptr)res->field->data;
	element_t *dst = (element_t *)res->data, *s0, *s1 = (element_t *)e->data, *s2 = (element_t *)f->data;
  ...
  s0 = (element_t *)p0->data;
  s1 = (element_t *)p1->data;
  s2 = (element_t *)p2->data;
  ...
  kar_poly_2(s2, s2[3], s2[4], s0, s1, (element_t *)p3->data);
  kar_poly_2(s0, s0[3], s0[4], a0, b0, (element_t *)p3->data);
  kar_poly_2(s1, s1[3], s1[4], a1, b1, (element_t *)p3->data);
...
static void polymod_mul(element_ptr res, element_ptr e, element_ptr f) {
  ...
  int n = p->n;
  element_t *dst;
  element_t *s1 = (element_t *)e->data, *s2 = (element_t *)f->data;
  element_t prod, p0, c0;
  int i, j;
  element_t *high;  // Coefficients of x^n, ..., x^{2n-2}.

  high = (element_t *)pbc_malloc(sizeof(element_t)* (n - 1));
  for (i=0; i<n-1; i++) {
    element_init(high[i], p->field);
    element_set0(high[i]);
  }
  element_init(prod, res->field);
  dst = (element_t *)prod->data;
  element_init(p0, res->field);
...
static void polymod_square_degree3(element_ptr res, element_ptr e) {
  ...
  // multiplication. If so convert to Karatsuba.
	element_t *dst = (element_t *)res->data;
	element_t *src = (element_t *)e->data;
  mfptr p = (mfptr)res->field->data;
  element_t p0;
  element_t c0, c2;
  element_ptr c1, c3;
...
static void compute_x_powers(field_ptr field, element_ptr poly) {
  ...
  for (i=1; i<n; i++) {
	  coeff = (element_t *)xpwr[i - 1]->data;
	  coeff1 = (element_t *)xpwr[i]->data;
...
void poly_set_coeff0(element_ptr e, int n) {
	peptr p = (peptr)e->data;
  if (n < p->coeff->count) {
    element_set0((element_s*)p->coeff->item[n]);
...
void poly_set_coeff1(element_ptr e, int n) {
  ...
  element_set1((element_s*)p->coeff->item[n]);
}
...
void field_init_polymod(field_ptr f, element_ptr poly) {
	pfptr pdp = (pfptr)poly->field->data;
  field_init(f);
  mfptr p;
  f->data = pbc_malloc(sizeof(*p));
  p = (mfptr)f->data;
  ...
  p->xpwr = (element_t*)pbc_malloc(sizeof(element_t)* n);
...
void polymod_const_mul(element_ptr res, element_ptr a, element_ptr e) {
  // a lies in R, e in R[x].
	element_t *coeff = (element_t *)e->data, *dst = (element_t *)res->data;
  int i, n = polymod_field_degree(e->field);
...
int poly_is_irred(element_ptr f) {
  ...
  struct checkgcd_scope_var v = {z, deg, basef, xpow, x, f, g};
