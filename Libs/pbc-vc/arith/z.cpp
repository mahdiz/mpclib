...
static void z_set0(element_ptr e) {
	mpz_set_ui((mpz_ptr)e->data, 0);
}

static void z_set1(element_ptr e) {
	mpz_set_ui((mpz_ptr)e->data, 1);
}
...
static void z_square(element_ptr c, element_ptr a) {
	mpz_mul((mpz_ptr)c->data, (mpz_ptr)a->data, (mpz_ptr)a->data);
}
...
static void z_pow_mpz(element_ptr n, element_ptr a, mpz_ptr z) {
	mpz_pow_ui((mpz_ptr)n->data, (mpz_ptr)a->data, mpz_get_ui(z));
}
...
static void z_neg(element_ptr n, element_ptr a) {
	mpz_neg((mpz_ptr)n->data, (mpz_ptr)a->data);
}

static void z_invert(element_ptr n, element_ptr a) {
	if (!mpz_cmpabs_ui((mpz_ptr)a->data, 1)) {
		mpz_set((mpz_ptr)n->data, (mpz_ptr)a->data);
	}
	else mpz_set_ui((mpz_ptr)n->data, 0);
}

static void z_div(element_ptr c, element_ptr a, element_ptr b) {
	mpz_tdiv_q((mpz_ptr)c->data, (mpz_ptr)a->data, (mpz_ptr)b->data);
}
...
static void z_from_hash(element_ptr n, void *data, int len) {
	mpz_import((mpz_ptr)n->data, len, -1, 1, -1, 0, data);
}
...
static int z_is_sqr(element_ptr a) {
	return mpz_perfect_power_p((mpz_ptr)a->data);
}

static void z_sqrt(element_ptr c, element_ptr a) {
	mpz_sqrt((mpz_ptr)c->data, (mpz_ptr)a->data);
}
...
static int z_length_in_bytes(element_ptr a) {
	return (mpz_sizeinbase((mpz_ptr)a->data, 2) + 7) / 8 + 4;
}
