...
static void fp_init(element_ptr e) {
  e->data = pbc_malloc(sizeof(unsigned long));
  unsigned long *p = (unsigned long *)e->data;
  *p = 0;
}
...
static void fp_set_mpz(element_ptr e, mpz_ptr z) {
  mpz_t r;
  mpz_init(r);
  unsigned long *p = (unsigned long *)e->field->data;
  unsigned long *l = (unsigned long *)e->data;
  mpz_fdiv_r_ui(r, z, *p);
  *l = mpz_get_ui(r);
  mpz_clear(r);
}
...
static void fp_set_si(element_ptr e, signed long int op) {
	unsigned long int *d = (unsigned long int *)e->data;
  unsigned long *p = (unsigned long *)e->field->data;
  if (op < 0) {
    *d = (-op) % *p;
    *d = *p - *d;
...
static void fp_to_mpz(mpz_ptr z, element_ptr e) {
	unsigned long int *l = (unsigned long int *)e->data;
  mpz_set_ui(z, *l);
}
...
static void fp_add(element_ptr c, element_ptr a, element_ptr b) {
	unsigned long *prime = (unsigned long *)a->field->data;
	unsigned long *p = (unsigned long *)a->data;
	unsigned long *q = (unsigned long *)b->data;
	unsigned long *r = (unsigned long *)c->data;
  unsigned long l0;
  l0 = *p + *q;
  if (l0 < *p) {
    //overflow
...
void field_init_tiny_fp(field_ptr f, mpz_t prime) {
  ...
  f->data = pbc_malloc(sizeof(unsigned long));
  p = (unsigned long*)f->data;
  *p = mpz_get_ui(prime);
  {
