...
{
	/// <summary>
	/// Implements the MPC protocol of Saia and Zamani 2013.
	/// </summary>
	public class CryptoMpcProtocol : MpcProtocol<BigZp>
	{
		#region Fields
		...
		protected IList<CommitmentMsg> RecvCommitments;
		protected readonly int ShamirPolyDegree;

		#endregion Fields

		public CryptoMpcProtocol(Entity e, BigCircuit circuit, ReadOnlyCollection<int> pIds,
			...
			: base(e, pIds, input, stateKey)
		{
			...
			ShamirPolyDegree = (int)Math.Ceiling(EntityCount / 3.0);
		}
		...
		{
			var evss = new eVSS(Entity, EntityIds, StateKey);
			evss.Run();
			...
			var shareValues = BigShamirSharing.Share(
				Input, EntityCount, ShamirPolyDegree, out coeffs);
			...
			var commitMsg = new CommitmentMsg(commitments, Stage.CommitmentsBroadcast);
			...
			OnReceive((int)Stage.CommitmentsBroadcast, EntityCount,
				delegate(List<CommitmentMsg> msgs)
				{
