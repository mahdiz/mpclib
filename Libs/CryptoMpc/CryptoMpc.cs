...
{	
	...
	public class CryptoMpc : SyncMpc<BigZp>
	{
		...
		public CryptoMpc(SyncParty p, IList<int> pIds, BigZp input, int seed)
			: base(p, pIds, input)
		{
			Prime = input.Prime;
			Seed = seed;
		...
		public override void Run()
		{
			...
			if (Party.Id == 0)
			{
				var polyDegree = (int)Math.Ceiling(PartyIds.Count / 3.0);
				var evss = new eVSS(Party, PartyIds, Prime, polyDegree - 1);
				evss.Setup(Seed);
				var shares = evss.ShareVerify(new BigZp(Prime, StaticRandom.Next(Prime - 1)), false);
