...
namespace MpcLib.Common.FiniteField.Circuits
{
	public class BigParser
	{
		...
		private BigWire outputWire;
		private BigInteger prime;
		private BigCircuit circuit;
		private IList<string> inputs = new List<string>();
		private Dictionary<string, BigWire> varsToWires;

		#endregion Fields

		public virtual BigCircuit Circuit
		{
			get
			{
				return circuit;
		...
		public BigParser(string pathOrString, BigInteger prime)
		{
			this.pathOrString = pathOrString;
			this.prime = prime;
		}

		public BigParser(FunctionType type, int numPlayers, BigInteger prime)
		...
		public BigCircuit CreateCircuit(BigInteger prime, IList<string> inputs)
		{
			var gates = new List<BigGate>();
			var currWires = new List<BigWire>() { outputWire };

			while (currWires.Count > 0)
			{
				var toRemove = new List<BigWire>();
				var toAdd = new List<BigWire>();
			...
			return new BigCircuit(prime, gates, inputs, outputWire.SourceGate);
		}
		...
		private BigWire GetWireFromCondition(string line)
		{
		...
		private BigWire CreateEqualsGate(string line)
		{
