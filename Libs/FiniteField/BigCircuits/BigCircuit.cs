...
namespace MpcLib.Common.FiniteField.Circuits
{
	public class BigCircuit
	{
		...
		public readonly BigGate OutputGate;
		protected internal BigInteger Prime;
		...
		private IList<BigWire> GetInputOrOutputWires(bool isInput) //todo - check that there are no duplicates
		{
			var inputOrOutputWires = new List<BigWire>();
			foreach (BigGate gate in Gates)
			{
				var wires = isInput ? gate.InputWires : gate.OutputWires;
				foreach (BigWire wire in wires)
				{
		...
		/// </summary>
		internal BigZp InternalCalculate(string inputs, BigInteger prime)
		{
			inputs = inputs.Replace(" ", "");	// remove white spaces
			string[] tokens = StringSplit(inputs, ",", true);
			var usersInputs = new List<BigZp>();

			for (int i = 0; i < tokens.Length; i++)
				usersInputs.Add(new BigZp(prime, Convert.ToInt32(tokens[i])));
		...
		//    public ExponentCircuit(BigInteger prime, int exp)
		//    {
		//        this.prime = prime;
		//        this.exp = exp;
		//    }
